...
# documents, and every OTHER cell in this suite takes `genDispatch` from `ci/flake.nix`, which
# builds it with `import ../lib` from ci's own inputs and so never evaluates the root shim. This is
# the cell that does — the L1 migration replaced this file's PREDECESSOR (the pre-arm-B shape) with
# the resolver-seam shape below: one dependency, `prelude`, wired the same three-channel way every
# other library in this roster now is.
#
...
#   prelude — `deriveGroup`'s classification fold runs through `prelude.unique` over the produced
#             kinds' classified groups, so forcing its result is what drives the call through
#             gen-prelude rather than merely returning the rule's caller-supplied `group` unread.
#
...
let
  ...
  # ★ THE ONE dependency formal, from the SAME binding `ci/flake.nix` builds its `lib` output from.
  # That is what keeps this cell offline, and it is also what makes the cell a reading of the
  ...
  # the one dependency formal is left at its default, which is the point — the default is the
  # subject.
  pathArgs = {
    dep = segs: segs;
    wire = args: args;
  ...
  # hermetic fixture is the only thing that can discriminate the resolver rule where a repo's own
...
in
{
  ...
  # supplies the dependency formal explicitly, so the shim's `fetch`-backed DEFAULT — which is where
  # the divergence lives — is never forced. Forcing it would put `builtins.fetchTree` inside the
  # suite. This cell reads the CONSTRUCTION instead of the outcome, which is strictly wider: it also
  # catches the member that never throws (a defaulted formal on the far side turns the loud arm of
  # the class silent) and the member that has not yet drifted.
  #
  ...
  # supply the `fetch`-backed formal, and that was a property of the file's TEXT which nothing
  ...
  # cells above hermetic. `entry` supplies the dependency formal, so the shim's `ci/flake.lock`-backed
  # default never fires there; nothing is supplied here.
  #
  ...
  flake.tests.entry.test-the-defaulted-entry-publishes-the-flake-surface =
    ...
    in
    {
      ...
      expected = builtins.attrNames genDispatch;
    };
  ...
  # `<…>`. `builtins.seq` of the dispatched root runs the shim's eager body, which forces the wired
  ...
  # never reaches into the dependency's own surface.
